...
def init_camera():
    ...
    if new_cap.isOpened() != True:
        print("camera error")
        return None
    return new_cap

def cap_img(cap, img, index):
    for i in range(8):
        ret,preview = cap.read()
        cv2.imshow("preview", preview)
        #cv2.imshow("temp" + str(index), preview)
        #cv2.setMouseCallback('temp' + str(index), mouse)
        cv2.waitKey(1)
    cap.grab()
    ret, img[index] = cap.retrieve()
    #cv2.imwrite(file_name+"_1.png", img1)

def wait_and_prevew_camera(c):
...
def get_cube_string(img):
    ...
    pts1 = np.array([[128,28],[533,30],[531,438],[125,431]], dtype = "float32")
    hsv_6_face = []
    angle_list = [90, -90, 90, 90, 90, -90]
    for i in range(6):
